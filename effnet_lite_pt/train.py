...
def main():
    args = get_args()
    Best_err = 20000000.0
    total_number = min(5, args.num_classes)
    # Log
    ...
    if args.eval:
        if args.eval_resume is not None:
            ...
            validate(model, device, args, all_iters=all_iters, total_number=total_number)
        exit(0)

    while all_iters < args.total_iters:
        Best_err, all_iters = train(Best_err, model, device, args, val_interval=args.val_interval, bn_process=False, all_iters=all_iters, total_number=total_number)
        validate(model, device, args, all_iters=all_iters, total_number=total_number)
    Best_err, all_iters = train(Best_err, model, device, args, val_interval=int(1280000/args.batch_size), bn_process=True, all_iters=all_iters, total_number=total_number)
    validate(model, device, args, all_iters=all_iters, total_number=total_number)
...
def train(Best_err, model, device, args, *, val_interval, bn_process=False, all_iters=None,total_number=5):
    ...
    for iters in range(1, val_interval + 1):
        ...
        prec1, prec5 = accuracy(output, target, topk=(1, total_number))

        Top1_err += 1 - prec1.item() / 100
        Top5_err += 1 - prec5.item() / 100

        if all_iters % args.display_interval == 0:
            if Top1_err < Best_err:
                printInfo = 'Top-1 err = {:.6f},\t'.format(Top1_err / args.display_interval) + \
                            'Best-model err = {:.6f},\t'.format(Best_err / args.display_interval)
    ...
    return Best_err, all_iters

def validate(model, device, args, *, all_iters=None, total_number=5):
    objs = AvgrageMeter()
    top1 = AvgrageMeter()
    top5 = AvgrageMeter()
